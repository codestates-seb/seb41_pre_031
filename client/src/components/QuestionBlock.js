	...
	const commentPost = () => {
		if (type === "question") {
			...
			try {
				...
				var config = {
					method: "post",
					url: `${ROOT_URL}/questions/${quId}/comments`,
					headers: {
						"Authorization": token,
			...
			try {
				...
				// const config = {
				//   method: "post",
				//   url: `${ROOT_URL}/questions/${answerID}/comments`,
				//   headers: {
				//     Authorization: token,
				//     "Content-Type": "application/json",
				//   },
			...
			<div className="layout">
				<QuestionVote>
					<div className="voteBox">
						...
						<div className="voteCount">{type === "question" ? data && data.questionLikes : data && data.answersLikes}</div>
						<button>
				...
				<QuestionPost>
					<div
						...
						dangerouslySetInnerHTML={{
							__html: type === "question" ? data && data.questionBody : data && data.answersBody,
						}}
					></div>
					...
					<div className="profileBox">
						...
						<UserProfile className="owner">
							<div className="userInBox">
								<div className="date">asked {type === "question" ? data && data.questionRegDate : data && data.answersRegDate}</div>
								<div className="userImg">
									<img src="https://i.stack.imgur.com/yZM8h.png?s=64&g=1" />
								</div>
								<div className="userData">
									<Link to={`/users/profile/${data && data.user[0].memberId}`}>{data && data.user[0].memberName}</Link>
											...
											<li key={index}>
												<div className="comment">
													...
													<Link to={`/users/profile/${data && data.userId}`}>
														<div className="commentUser">-{data && data.name}</div>
													</Link>
