...
export async function userProfileData(id) {
	...
	const 내답변 = answersList.data.data.filter((data) => data.memberName === member.data.data.memberName);
...
export async function questionData(id) {
	...
	const user = users.data.data.filter((data) => data.memberName === question.data.data.memberName);
...
export async function answerData(id) {
	...
	const users = await axios.get(`${host}/members`);
	const filteranswers = answers.data.data.filter((data) => String(data.questionId) === id);
