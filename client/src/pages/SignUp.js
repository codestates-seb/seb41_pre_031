...
const SignUp = ({ setFlag, setIsFooter }) => {
	...
	const signUpServer = () => {
		...
		if (!email) {
			setErrorMsg({ ...errorMsg, email: "Email cannot be empty." });
			return;
		}
		if (!password) {
			setErrorMsg({ ...errorMsg, password: "Password cannot be empty." });
			return;
		}
		if (!/(?=.*\d)(?=.*[a-zA-Z]).{8}/.test(password)) {
			setErrorMsg({ ...errorMsg, password: "Please check your password." });
			return;
		}
		return axios
				...
				<LoginSignUpStyle.ContentBox>
					<form onSubmit={(e) => e.preventDefault()}>
						<LoginSignUpStyle.InputWrap>
							...
							<input type="text" maxLength="30" onChange={(e) => handleInputValue("memberName", e)} id="displayName" />
						</LoginSignUpStyle.InputWrap>
						<LoginSignUpStyle.InputWrap>
							<label htmlFor="email">Email</label>
							<input type="email" onChange={(e) => handleInputValue("email", e)} id="email" className={errorMsg.email === "" ? "" : "errorInput"} />
							<p className="errorMessage">{errorMsg.email}</p>
						</LoginSignUpStyle.InputWrap>
						<LoginSignUpStyle.InputWrap>
							<label htmlFor="password">Password</label>
							<input type="password" maxLength="20" onChange={(e) => handleInputValue("password", e)} id="password" className={errorMsg.password === "" ? "" : "errorInput"} />
						...
						<button onClick={signUpServer} className="btnPrimary">
							Sign up
						</button>
					</form>
					<Desc>
